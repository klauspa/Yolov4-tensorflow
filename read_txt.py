from conf import MAX_TRUE_BOX_NUM_PER_IMG
...
class ReadTxt(object):
    ...
    def parse_line(self):
        ...
        for i in range(MAX_TRUE_BOX_NUM_PER_IMG):
            if i < box_num:
                box_xmin = float(split_line[i * 5 + 1])
                box_ymin = float(split_line[i * 5 + 2])
                box_xmax = float(split_line[i * 5 + 3])
                box_ymax = float(split_line[i * 5 + 4])
                class_id = int(split_line[i * 5])
                boxes.append([class_id, box_xmin, box_ymin, box_xmax, box_ymax])
            """
            else:
                box_xmin = 0
                box_ymin = 0
